...
export function ExecutionTimeline({ filters }: { filters?: { status?: string; agent?: string } }) {
  ...
  if (isLoading) return <p className="text-sm text-muted-foreground py-8 text-center">Loading timeline…</p>;
  if (groups.length === 0) {
    return (
              ...
              <div className="flex items-center gap-1.5 min-w-0 flex-1">
                {group.entries.slice(0, 5).map((e, i) => (
                  <span key={e.id} className="flex items-center gap-1">
                    ...
                    <span className="text-xs font-mono truncate max-w-[120px]">{e.skill_name ?? 'unknown'}</span>
                  </span>
                ))}
                {group.entries.length > 5 && <span className="text-xs text-muted-foreground">+{group.entries.length - 5}</span>}
              </div>
              <div className="flex items-center gap-2 shrink-0">
                {group.entries.length > 1 && <Badge variant="secondary" className="text-[10px]">{group.entries.length} steps</Badge>}
                {group.hasFailure && <AlertCircle className="h-3.5 w-3.5 text-destructive" />}
                {group.hasPending && <Clock className="h-3.5 w-3.5 text-amber-500" />}
              ...
              <CardContent className="pt-0 pb-4 px-4">
                <div className="relative ml-6 border-l border-border pl-6 space-y-0">
                  {group.entries.map((entry, idx) => {
                    ...
                    return (
                      <div key={entry.id} className={`relative ${idx < group.entries.length - 1 ? 'pb-4' : ''}`}>
                        <div className={`absolute -left-[31px] top-1 h-3 w-3 rounded-full border-2 border-background ${STATUS_DOT[entry.status]}`} />
                        <button
                        ...
                        >
                          <div className="flex-1 min-w-0">
                            <div className="flex items-center gap-2">
                              <span className="text-sm font-mono font-medium">{entry.skill_name ?? 'unknown'}</span>
                              <Badge variant="outline" className="text-[10px]">{entry.agent}</Badge>
                              <Badge variant="outline" className={`text-[10px] ${entry.status === 'failed' ? 'border-destructive/30 text-destructive' : entry.status === 'pending_approval' ? 'border-amber-400/30 text-amber-600' : ''}`}>
                          ...
                          {entry.status === 'pending_approval' && (
                            <div className="flex gap-1 shrink-0" onClick={(e) => e.stopPropagation()}>
                              <Button size="icon" variant="ghost" className="h-7 w-7" onClick={() => approve.mutate({ id: entry.id, approved: true })}><Check className="h-4 w-4 text-primary" /></Button>
                              <Button size="icon" variant="ghost" className="h-7 w-7" onClick={() => approve.mutate({ id: entry.id, approved: false })}><X className="h-4 w-4 text-destructive" /></Button>
                            </div>
                          )}
                        </button>
                        {isEntryExpanded && (
                          <div className="mt-2 mx-2 grid grid-cols-2 gap-3 text-xs">
                            <div>
                              ...
                              <pre className="mt-1 p-2 rounded bg-muted font-mono text-[11px] overflow-auto max-h-36">{JSON.stringify(entry.input, null, 2)}</pre>
                            </div>
                            <div>
                              <span className="font-medium text-muted-foreground">Output</span>
                              <pre className="mt-1 p-2 rounded bg-muted font-mono text-[11px] overflow-auto max-h-36">{JSON.stringify(entry.output, null, 2)}</pre>
                            </div>
                          </div>
                        )}
                      </div>
                ...
                {group.entries.length > 1 && (
                  <div className="mt-3 pt-3 border-t flex items-center gap-4 text-xs text-muted-foreground ml-6">
                    <span>{group.entries.filter(e => e.status === 'success').length}/{group.entries.length} succeeded</span>
                    <span>Total: {group.totalDuration}ms</span>
                    <span>Span: {formatDistanceStrict(new Date(group.firstAt), new Date(group.lastAt))}</span>
                  </div>
                )}
              </CardContent>
            )}
